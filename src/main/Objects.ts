...
export const deepExtend = function( ...sources: any[] ): any{
	if( sources.length == 1 )
		return sources[ 0 ];
	if( sources.length == 0 )
		return;
	...
	for( let i = 1; i < sources.length; i++ ){
		...
		if( !( typeof add == 'object' ) ){
			...
			if( !( add instanceof Array ) ){
				for( let x in add ){
					source[x] = deepExtend(source[x], add[x]);
				}
			}else{
				for( let a = 0; a < add.length; a++ ){
					source[a] = deepExtend(source[a], add[a]);
				}
			}
		}
	}
...
export const getNestedValue = function( obj: any, ...keys: string[] ): any{
    if( keys.length === 1 ){
        return obj[ keys[0] ];
    }else{
        let value: any = obj;
...
export const setNestedValue = function( obj: any, value: any, ...keys: string[] ): any{
